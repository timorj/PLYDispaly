...
namespace DataLoad
{
    class Model
    {
        ...
        private Mesh processMesh(Assimp.Mesh mesh, Scene scene)
        {
            ...
            List<DataLoad.Mesh.Texture> textures = new List<DataLoad.Mesh.Texture>();
            ...
            if (mesh.MaterialIndex > 0)
            {
                ...
                List<DataLoad.Mesh.Texture> diffuseMaps = loadMatericalTextures(material, TextureType.Diffuse, "diffuse");
                for (int i = 0; i < diffuseMaps.Count; i++)
                {
                    textures.Insert(textures.Count, diffuseMaps[i]);
                }
                List<DataLoad.Mesh.Texture> specularMaps = loadMatericalTextures(material, TextureType.Specular, "specular");
        ...
        /// <returns></returns>
        private List<DataLoad.Mesh.Texture> loadMatericalTextures(Material material, TextureType type, string typename)
        {
        ...
        private int TextureFromFile(string path, string directory)
        {
            try
            {
                ...
                if (data != null)
                {
                    ...
                    GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureWrapS, (int)OpenTK.Graphics.ES10.TextureWrapMode.Repeat);
                    GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureWrapT, (int)OpenTK.Graphics.ES10.TextureWrapMode.Repeat);
